...
- **Usabilidade reintroduzida em 24/09/2026** (branch `feature/usabilidade-monitoramento`, feita pelo Hugo em 21/09 e nunca publicada — o redesenho de 22/09 e os 16 pares foram feitos por cima de uma base sem ela): bloco de status com orientação (`mensagens_de_status`), barra de espera animada/que enche até o próximo fechamento (`componentes.BarraEspera`, "Reduzir movimento"), "sem sinal" escrito no quadro, "Ver exemplo de alerta" (usa o pop-up real com título SIMULAÇÃO, sem passar pela fila/CSV), "Como usar", "Abrir registros", aviso na primeira vez que o X manda pra bandeja, motor que morre sozinho devolve a janela ao estado "pode iniciar" (`executar_motor` + laço `_verificar_fila` permanente) e, **o mais importante: `PASTA_LOGS` no .exe vai pra `%APPDATA%\AlertaFuturos\logs`** — com `Path(__file__)` o PyInstaller --onefile gravava o CSV na pasta temporária `_MEIxxxx`, apagada ao fechar o app. Testes em `test_usabilidade.py` (os de janela real precisam de tela: `xvfb-run` no Linux). Empilhado (tela < 1280 px), enquanto roda, a regra vira uma linha de resumo pra o quadro e o rodapé caberem em 1024x768.
- **Tela cheia (24/09/2026, pedido do cliente: "janela até da tela toda")**: a janela deixou de ser `resizable(False, False)`. Maximizar (botão da janela, link "Tela cheia" ou F11) refaz o conteúdo inteiro ampliado (`Tema.zoom`, aplicado em `px()` e nos tamanhos de fonte), com o zoom calculado por `zoom_para()` a partir do tamanho natural medido na abertura — 1920×1080 dá 1,5×. `aplicar_zoom()` destrói e remonta só o `self.conteudo` e restaura campos, rótulos, estado da sessão e a última avaliação de cada par (`self._ultimas`); motor, fila e CSV não são tocados. Em 1366×768 o ganho é < 8% e a janela só maximiza, sem refazer.
- **Atualização automática (24/09/2026, pedido do Lucas, aprovado pelo Hugo)**: `atualizacao.py` + `versao.py` + `publicar_versao.py`. Só no `.exe` (`sys.frozen`). 4 s após abrir, thread consulta `/versao.json` (timeout 6 s, qualquer erro = silêncio). Versão mais nova → `askyesno` (só com a janela visível; quem abriu minimizado é perguntado ao abrir a janela). Download para `%APPDATA%\AlertaFuturos\atualizacao`, conferindo tamanho, SHA-256, cabeçalho `MZ` e URL obrigatoriamente em futuros.blumenauti.com.br. Troca: `os.replace` do `.exe` em uso para `Versões anteriores\AlertaFuturos-<versão>.exe` (Windows permite renomear exe em execução no mesmo disco), novo no mesmo caminho (atalho e Registro do "iniciar com o Windows" continuam válidos), desfaz se o 2º passo falhar; reabre com `PYINSTALLER_RESET_ENVIRONMENT=1` (senão o onefile filho reaproveita a pasta `_MEI` do pai, que é apagada). "Não" deixa link dourado no rodapé. Falha → oferece a página de download. Sem assinatura digital por decisão (próxima versão).
